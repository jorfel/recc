...
static handle_holder process_from_id(DWORD pid)
{
    ...
    if(!hprocess)
        throw win32_error(GetLastError(), "OpenProcess failed.");
...
{
    std::pair<HWND, const std::wstring*> vars = { 0, &cmptitle }; //store as pair so a single ptr can be supplied to EnumWindows
    ...
    if(GetWindowThreadProcessId(vars.first, &pid) == 0)
        throw win32_error(GetLastError(), "GetWindowThreadProcessId failed.");
...
/// Coroutine for creating a pipe and directing it to output file f.
static void_task print_pipe(signal_context &ctx, const std::wstring &pipe_path, FILE *f)
{
    ...
    if(!hpipe)
        throw win32_error(GetLastError(), "CreateNamedPipeW failed.");

    handle_holder hpipe_event(CreateEventW(nullptr, TRUE, FALSE, nullptr));
    if(!hpipe)
        throw win32_error(GetLastError(), "CreateEventW failed.");
    ...
    if(!ConnectNamedPipe(hpipe.get(), &pipe_async) && GetLastError() != ERROR_IO_PENDING) //get notified when client connects
        throw win32_error(GetLastError(), "ConnectNamedPipe failed.");
    ...
    for(;;)
    {
        if(!ReadFile(hpipe.get(), read_buffer, sizeof(read_buffer), nullptr, &pipe_async))
        {
            ...
            if(err == ERROR_BROKEN_PIPE) //client disconnected
                ...
                throw win32_error(GetLastError(), "ReadFile failed.");
        }
        ...
        if(!GetOverlappedResult(hpipe.get(), &pipe_async, &bytes_avail, FALSE))
        {
            ...
            if(err == ERROR_BROKEN_PIPE) //client disconnected
                ...
                throw win32_error(GetLastError(), "GetOverlappedResult failed.");
        }
...
static void_task capture(signal_context &ctx, HANDLE hprocess, const std::wstring &dllpath, const std::string &api, const std::string &format, const std::wstring &out_path, const std::wstring &log_path)
{
    ...
    if(DWORD c = co_await thread_awaiter(ctx, hthread.get()); c != 0)
        throw win32_error(c, "Thread for recc_log reported failure.");

    //call recc_capture
    hthread = dll_call(hprocess, false, dllpath, "recc_capture", out_path, api, format);
    if(DWORD c = co_await thread_awaiter(ctx, hthread.get()); c != 0)
        throw win32_error(c, "Thread for recc_capture reported failure.");
    ...
    if(DWORD c = co_await thread_awaiter(ctx, hthread.get()); c != 0)
        throw win32_error(c, "Thread for recc_release reported failure.");
}

/// Creates a path that points to the DLL file within the exe's directory.
static std::wstring make_dll_path()
{
    wchar_t exepath[MAX_PATH];
    auto flen = GetModuleFileNameW(0, exepath, MAX_PATH);
    if(flen == 0)
        throw win32_error(GetLastError(), "GetModuleFileNameW failed.");

    auto exepath_str = std::wstring(exepath, flen);
    exepath_str.erase(exepath_str.find_last_of(L'\\') + 1);
    return exepath_str + L"recc_dll.dll";
}
...
static int main2(int argc, char **argv)
{
    ...
    if(out == "--") //when outputting to stdout, create a pipe with unique name
    {
    ...
    captask = capture(main_loop, hprocess.get(), make_dll_path(), optres["api"].as<std::string>(), optres["format"].as<std::string>(), out_path, log_path);
    main_loop.run();
    return 0;
}
